...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
#define AUDIO_BUFFER_SIZE       4096   // Kích thước buffer RAM
#define HEADER_OFFSET           78     // Header wav cần bỏ qua
/* USER CODE END PM */
...
int16_t i2s_audio_buffer[AUDIO_BUFFER_SIZE / 2]; // Bộ đệm RAM cho DMA
uint32_t music_cursor = 0;                       // Con trỏ đọc nhạc
/* USER CODE END PV */
...
int main(void)
{
  ...
  Audio_Fill_Buffer((uint8_t*)i2s_audio_buffer, AUDIO_BUFFER_SIZE);
  HAL_I2S_Transmit_DMA(&hi2s2, (uint16_t*)i2s_audio_buffer, AUDIO_BUFFER_SIZE / 2);
  /* USER CODE END 2 */
...
static void MX_I2S2_Init(void)
{
  ...
  hi2s2.Init.DataFormat = I2S_DATAFORMAT_16B; // Giữ nguyên cấu hình chuẩn
